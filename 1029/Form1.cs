...
namespace _1029
{
    public partial class terminal : Form
    {
        ...
        static string files_roads = @"доп.файлы\файлы и ссылки\roads.txt";
        static string files_names = @"доп.файлы\файлы и ссылки\names.txt";
        ...
        private void file_writer_Tick(object sender, EventArgs e)
        {
            ...
            {
                File.Delete(@"доп.файлы\comand_txt.txt");
            ...
            else if (input.Text.Contains("\\"))
            {
                ...
                if (input.Text.Contains("C:\\") || input.Text.Contains("D:\\"))
                {
                    if (input.Text.Contains("/"))
                    {
                        if (input.Text.Contains("add"))
                        {
                            ...
                            File.WriteAllText(@"доп.файлы\comand_txt.txt", temp);
                            string[] file = File.ReadAllLines(@"доп.файлы\comand_txt.txt");
                ...
                else
                {
                    ...
                    if (input.Text.Contains("name") && input.Text.Contains("id"))
                    {
                        ...
                        File.WriteAllText(@"доп.файлы\comand_txt.txt", temp);
                        string[] file = File.ReadAllLines(@"доп.файлы\comand_txt.txt");
                        string temp_name = File.ReadAllText(@"доп.файлы\файлы и ссылки\names_fm.txt");
                        string temp_id = File.ReadAllText(@"доп.файлы\файлы и ссылки\ides.txt");
                        if (temp_name != null)
                        {
                            temp_name += "\n" + file[0];
                            temp_id += "\n" + file[1];
                        ...
                        File.WriteAllText(@"доп.файлы\файлы и ссылки\names_fm.txt", temp_name);
                        File.WriteAllText(@"доп.файлы\файлы и ссылки\ides.txt", temp_id);
                    ...
                    if(input.Text[0] == 'M' || input.Text[0] == 'm')
                    {
                        try
                        {
                            ...
                            string temp_name = File.ReadAllText(@"доп.файлы\файлы и ссылки\names_fm.txt");
                            string temp_id = File.ReadAllText(@"доп.файлы\файлы и ссылки\ides.txt");
                            string[] temp_id_arr = File.ReadAllLines(@"доп.файлы\файлы и ссылки\ides.txt");
                            string[] temp_name_arr = File.ReadAllLines(@"доп.файлы\файлы и ссылки\names_fm.txt");

                            if (temp_name != null)
                            {
                                for (int i = 1; i < File.ReadAllLines(@"доп.файлы\файлы и ссылки\names_fm.txt").Length; i++)
                                {
                                    if (input.Text.Contains(temp_name_arr[i]))
                                    {
                                        ...
                                        try
                                        {
                                            File.WriteAllText(@"доп.файлы\bot\text.txt", file[0]);
                                            File.WriteAllText(@"доп.файлы\bot\id.txt", temp_id_arr[i].ToString());

                                            ProcessStartInfo startInfo = new ProcessStartInfo("python");
                                            Process process = new Process();
                                            startInfo.WorkingDirectory = @"доп.файлы\bot";
